...
export default function ProfileScreen() {
  ...
  const [targetResults, setTargetResults] = useState<Profile[]>([]);
  ...
  const loadProfile = async () => {
    ...
    const { data: sent } = await supabase.from('breakthroughs')
      .select('*, profiles!breakthroughs_target_id_fkey(username, avatar_colors)')
      .eq('sender_id', user.id).order('created_at', { ascending: false });
    if (sent) setBreakthroughs(sent as Breakthrough[]);

    const { data: recv } = await supabase.from('breakthroughs')
      .select('*, profiles!breakthroughs_sender_id_fkey(username, avatar_colors)')
      .eq('target_id', user.id).order('created_at', { ascending: false });
    if (recv) setReceived(recv as Breakthrough[]);
  };

  const regenerateAvatar = async () => {
    if (!profile) return;
    const colors = await generateAvatarColors(profile.username);
    await supabase.from('profiles').update({ avatar_colors: colors }).eq('id', profile.id);
    setProfile({ ...profile, avatar_colors: colors });
  };

  const searchTarget = async (q: string) => {
    if (!q.trim()) { setTargetResults([]); return; }
    const { data } = await supabase.from('profiles').select('id, username, avatar_colors')
      .ilike('username', `%${q}%`).limit(8);
    if (data) setTargetResults(data as Profile[]);
  };
  ...
  const polishContent = async () => {
    ...
    const res = await client.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: '你是一个文案专家，将用户的事迹润色为简洁有力的"高光简报"，适合展示给目标人物。保留核心事实，语气自信不浮夸，100字以内。只返回润色后的文案。',
        },
  ...
  const sendBreakthrough = async () => {
    if (!selectedTarget || !polished) return;
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return;
  ...
  const likeBreakthrough = async (btId: string) => {
    await supabase.from('breakthroughs').update({ is_liked: true }).eq('id', btId);
    setReceived((prev) => prev.map((b) => b.id === btId ? { ...b, is_liked: true } : b));
  };

  const avatarColors = (profile?.avatar_colors ?? ['#7C3AED', '#EC4899', '#F59E0B']) as [string, string, string];
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <Animated.View entering={FadeInDown} style={styles.avatarSection}>
          ...
          <Text style={styles.energyScore}>能量值 {profile?.energy_score ?? 0}</Text>
        </Animated.View>
          ...
          <View style={styles.section}>
            ...
            {received.map((bt) => (
              <View key={bt.id} style={styles.btCard}>
                <Text style={styles.btFrom}>来自 {bt.profiles?.username}</Text>
                <Text style={styles.btContent}>{bt.polished_content}</Text>
                {!bt.is_liked && (
                ...
                {bt.is_liked && <Text style={styles.likedTag}>已点亮 ✦</Text>}
              </View>
            ))}
          </View>
        )}
        ...
        {breakthroughs.length > 0 && (
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>已发送的简报 ({breakthroughs.length})</Text>
            {breakthroughs.map((bt) => (
              <View key={bt.id} style={[styles.btCard, styles.btSent]}>
                <Text style={styles.btFrom}>→ {bt.profiles?.username}</Text>
                <Text style={styles.btContent} numberOfLines={2}>{bt.polished_content}</Text>
                <Text style={[styles.btStatus, bt.is_liked && styles.btStatusLiked]}>
                  {bt.is_liked ? '已被点亮 ✦' : '等待回应...'}
                </Text>
              </View>
            ))}
          </View>
      ...
      <Modal visible={showBreakthrough} animationType="slide" presentationStyle="pageSheet">
        <View style={styles.modal}>
          <View style={styles.modalHeader}>
            <TouchableOpacity onPress={() => setShowBreakthrough(false)}>
          ...
          <ScrollView style={styles.modalScroll}>
            ...
            <TextInput
              ...
              value={targetSearch}
              onChangeText={(t) => { setTargetSearch(t); searchTarget(t); }}
            />
            {targetResults.map((u) => (
              <TouchableOpacity
                key={u.id}
                style={[styles.targetRow, selectedTarget?.id === u.id && styles.targetRowSelected]}
            ...
            <TextInput
              ...
              placeholder="描述你想展示的成就或故事..."
              placeholderTextColor={Colors.textMuted}
              value={btContent}
              onChangeText={setBtContent}
              multiline
            />

            <TouchableOpacity
              style={[styles.polishBtn, (!btContent.trim() || polishing) && { opacity: 0.4 }]}
              onPress={polishContent}
              disabled={!btContent.trim() || polishing}
            >
...
const styles = StyleSheet.create({
  ...
  polishBtn: {
    backgroundColor: 'rgba(192,132,252,0.15)', borderRadius: 14,
  ...
  polishedBox: {
    ...
    marginTop: 16, borderWidth: 1, borderColor: 'rgba(192,132,252,0.3)', gap: 10,
  },
